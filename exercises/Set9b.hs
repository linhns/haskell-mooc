...
prettyPrint n coords =
  let helper (i, j) ((x, y) : coords)
        | i == x && j == y = 'Q' : helper (nextCol (i, j)) coords
      helper (i, j) coords
        | i > n = ""
        | j > n = '\n' : helper (nextRow (i, j)) coords
        | otherwise = '.' : helper (nextCol (i, j)) coords
   in helper (1, 1) $ sort coords
...
danger c s = foldr (||) False $ map (attackable c) s

attackable :: Candidate -> Coord -> Bool
attackable c q = sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q
...
prettyPrint2 n qs = traverse (1, 1) $ sort qs
  where
    traverse (i, j) qs
      | i > n = ""
      | j > n = '\n' : traverse (nextRow (i, j)) qs
      | elem (i, j) qs = 'Q' : traverse (nextCol (i, j)) qs
      | danger (i, j) qs = '#' : traverse (nextCol (i, j)) qs
      | otherwise = '.' : traverse (nextCol (i, j)) qs
...
continue [] = [(1, 1)]
continue ((i, j) : qs) = nextRow (i, j) : ((i, j) : qs)

backtrack :: Stack -> Stack
backtrack [] = []
backtrack [(i, j)] = []
backtrack ((i, j) : (k, l) : qs) = nextCol (k, l) : qs
