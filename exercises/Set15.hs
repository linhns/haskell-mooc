...
validateAddress streetName streetNumber postCode =
  ...
  where
    checkStreetName name = check (length name <= 20) "Invalid street name" name
    checkStreetNumber number = check (all isDigit number) "Invalid street number" number
    checkPostCode postCode = check (length postCode == 5 && all isDigit postCode) "Invalid postcode" postCode
...
boolOrInt s = parseBool s <|> parseInt s
  where
    parseBool x = case readMaybe x :: Maybe Bool of
      Just b -> check True "" (Left b)
      Nothing -> invalid "Not a Bool"
    parseInt x = case readMaybe x :: Maybe Int of
...
normalizePhone s = checkLength trimmed *> traverse checkDigit trimmed
  where
    trimmed = filter (/= ' ') s
...
parseExpression input = case words input of
  ...
  where
    ...
    parseVar arg = check (length arg == 1 && isAlpha (head arg)) ("Invalid variable: " ++ arg) (Variable $ head arg)
...
instance Functor Priced where
  fmap f (Priced val p) = Priced val $ f p
