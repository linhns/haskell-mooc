...
blendColor (Color a b c) (Color x y z) = Color d e f
  where
    d = (a + x) `div` 2
    e = (b + y) `div` 2
    f = (c + z) `div` 2
...
combine f (Picture a) (Picture b) = Picture (\p -> f (a p) (b p))
...
union (Shape f) (Shape g) = Shape (\p -> f p || g p)

cut :: Shape -> Shape -> Shape
cut (Shape f) (Shape g) = Shape (\p -> f p && (not $ g p))
...
paintSolid color (Shape s) (Picture f) = Picture g
  where
    g c = if s c then color else f c
...
paint (Picture f) (Shape p) (Picture g) = Picture h
  where
    h c = if p c then f c else g c
...
instance Transform Blur where
  apply _ (Picture f) = let g (Coord x y) = avgColors (map (f . co) $ neighbours x y) in Picture g
    where
      co (x, y) = Coord x y
      neighbours x y = [(x, y), (x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]

avgColors :: [Color] -> Color
avgColors [] = Color 0 0 0
avgColors xs = Color (div (sum $ map getRed xs) l) (div (sum $ map getGreen xs) l) (div (sum $ map getBlue xs) l)
  where
    l = length xs
