...
countValid xs = length [x | x <- xs, (x !! 2) == (x !! 4) || (x !! 3) == (x !! 5)]
...
repeated [] = Nothing
repeated [x] = Nothing
repeated (x : xs) = if head xs == x then Just x else repeated xs
...
isOpen :: Lock -> Bool
isOpen (Lock _ status) = not status

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open code (Lock pass status) = if code == pass then Lock pass False else Lock pass status

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock pass _) = Lock pass True

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode _ (Lock pass True) = Lock pass True
changeCode code (Lock pass False) = Lock code False
...
instance Eq Text where
  (Text a) == (Text b) = rmSpace a == rmSpace b
    where
      rmSpace s = foldr (++) "" $ words s
