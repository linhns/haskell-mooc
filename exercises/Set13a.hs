...
checkNumber (s, t) = if any isDigit s || any isDigit t then Nothing else Just (s, t)
...
balance accountName = BankOp f
  where
    f (Bank accounts) = (Map.findWithDefault 0 accountName accounts, Bank accounts)
...
paren c = do
  ...
  if s == -1
    then put $ s
    else if c == '(' then put $ (s + 1) else put $ (s - 1)
